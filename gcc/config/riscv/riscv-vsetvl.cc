...
DEBUG_FUNCTION void debug(const vector_insn_info *info) { info->dump(stderr); }

DEBUG_FUNCTION void debug(const vector_infos_manager *info) {
  info->dump(stderr);
}

static bool vlmax_avl_p(rtx x) { return x && rtx_equal_p(x, RVV_VLMAX); }

static bool vlmax_avl_insn_p(rtx_insn *rinsn) {
...
/* Return true if the block is a loop itself:
        ...
        |________|     |
          ...
          reaching_out
*/
static bool loop_basic_block_p(const basic_block cfg_bb) {
...
static bool vsetvl_discard_result_insn_p(rtx_insn *rinsn) {
  if (!vector_config_insn_p(rinsn))
    return false;
...
static bool real_insn_and_same_bb_p(const insn_info *insn, const bb_info *bb) {
  return insn != nullptr && insn->is_real() && insn->bb() == bb;
}

static bool before_p(const insn_info *insn1, const insn_info *insn2) {
  return insn1->compare_with(insn2) < 0;
}

static insn_info *find_reg_killed_by(const bb_info *bb, rtx x) {
...
static rtx get_vl(rtx_insn *rinsn) {
  if (has_vl_op(rinsn)) {
...
static bool has_vsetvl_killed_avl_p(const bb_info *bb,
...
*/
static bool anticipatable_occurrence_p(const bb_info *bb,
...
*/
static bool available_occurrence_p(const bb_info *bb,
                                   const vector_insn_info dem) {
  insn_info *insn = dem.get_insn();
  /* The only possible operand we care of VSETVL is AVL.  */
  if (dem.has_avl_reg()) {
...
static bool insn_should_be_added_p(const insn_info *insn, unsigned int types) {
...
static hash_set<set_info *> get_all_sets(phi_info *phi, unsigned int types) {
...
static rtx get_avl(rtx_insn *rinsn) {
...
static set_info *get_same_bb_set(hash_set<set_info *> &sets,
...
static hash_set<basic_block> get_all_predecessors(basic_block cfg_bb) {
...
static bool any_set_in_bb_p(hash_set<set_info *> sets, const bb_info *bb) {
...
static uint8_t get_sew(rtx_insn *rinsn) { return get_attr_sew(rinsn); }

/* Helper function to get VLMUL operand. We always have VLMUL value for
   all RVV instructions that have VTYPE OP. */
static enum vlmul_type get_vlmul(rtx_insn *rinsn) {
  return (enum vlmul_type)get_attr_vlmul(rinsn);
}

/* Get default tail policy.  */
static bool get_default_ta() {
...
static bool tail_agnostic_p(rtx_insn *rinsn) {
...
static bool mask_agnostic_p(rtx_insn *rinsn) {
...
static bool has_vector_insn(function *fn) {
  basic_block cfg_bb;
  rtx_insn *rinsn;
...
static rtx gen_vsetvl_pat(enum vsetvl_type insn_type, const vl_vtype_info &info,
                          rtx vl) {
  rtx avl = info.get_avl();
...
static rtx gen_vsetvl_pat(rtx_insn *rinsn, const vector_insn_info &info,
                          rtx vl = NULL_RTX) {
  rtx new_pat;
  vl_vtype_info new_info = info;
  if (info.get_insn() && info.get_insn()->rtl() &&
      fault_first_load_p(info.get_insn()->rtl()))
    new_info.set_avl_info(avl_info(get_avl(info.get_insn()->rtl()), nullptr));
...
static void emit_vsetvl_insn(enum vsetvl_type insn_type,
                             enum emit_type emit_type,
                             const vl_vtype_info &info, rtx vl,
                             rtx_insn *rinsn) {
...
static void eliminate_insn(rtx_insn *rinsn) {
  if (dump_file) {
    fprintf(dump_file, "\nEliminate insn %d:\n", INSN_UID(rinsn));
...
static vsetvl_type insert_vsetvl(enum emit_type emit_type, rtx_insn *rinsn,
                                 const vector_insn_info &info,
                                 const vector_insn_info &prev_info) {
  ...
  if (vlmax_avl_p(info.get_avl())) {
    ...
    /* For user vsetvli a5, zero, we should use get_vl to get the VL
       operand "a5".  */
    rtx vl_op = vsetvl_insn_p(rinsn) ? get_vl(rinsn) : info.get_avl_reg_rtx();
...
static void add_label_notes(rtx x, rtx_insn *rinsn) {
...
static void insert_insn_end_basic_block(rtx_insn *rinsn, basic_block cfg_bb) {
...
static vl_vtype_info get_vl_vtype_info(const insn_info *insn) {
  set_info *set = nullptr;
  rtx avl = ::get_avl(insn->rtl());
  if (avl && REG_P(avl)) {
    if (vlmax_avl_p(avl) && has_vl_op(insn->rtl()))
      set = find_access(insn->uses(), REGNO(get_vl(insn->rtl())))->def();
...
static void change_insn(rtx_insn *rinsn, rtx new_pat) {
...
static const insn_info *get_forward_read_vl_insn(const insn_info *insn) {
...
static bool change_insn(function_info *ssa, insn_change change, insn_info *insn,
                        rtx new_pat) {
...
static void change_vsetvl_insn(const insn_info *insn,
                               const vector_insn_info &info,
                               rtx vl = NULL_RTX) {
  rtx_insn *rinsn;
...
static void local_eliminate_vsetvl_insn(const vector_insn_info &dem) {
  ...
  if (vsetvl_insn_p(rinsn)) {
    ...
    for (insn_info *i = insn->next_nondebug_insn();
         real_insn_and_same_bb_p(i, bb); i = i->next_nondebug_insn()) {
      if (i->is_call() || i->is_asm() || find_access(i->defs(), VL_REGNUM) ||
...
static bool source_equal_p(insn_info *insn1, insn_info *insn2) {
  if (!insn1 || !insn2)
    return false;
...
static insn_info *extract_single_source(set_info *set) {
  if (!set)
    return nullptr;
  if (set->insn()->is_real())
    return set->insn();
...
static unsigned calculate_sew(vlmul_type vlmul, unsigned int ratio) {
...
static vlmul_type calculate_vlmul(unsigned int sew, unsigned int ratio) {
...
static bool incompatible_avl_p(const vector_insn_info &info1,
                               const vector_insn_info &info2) {
  return !info1.compatible_avl_p(info2) && !info2.compatible_avl_p(info1);
}

static bool different_sew_p(const vector_insn_info &info1,
                            const vector_insn_info &info2) {
  return info1.get_sew() != info2.get_sew();
}

static bool different_lmul_p(const vector_insn_info &info1,
                             const vector_insn_info &info2) {
  return info1.get_vlmul() != info2.get_vlmul();
}

static bool different_ratio_p(const vector_insn_info &info1,
                              const vector_insn_info &info2) {
  return info1.get_ratio() != info2.get_ratio();
}

static bool different_tail_policy_p(const vector_insn_info &info1,
                                    const vector_insn_info &info2) {
  return info1.get_ta() != info2.get_ta();
}

static bool different_mask_policy_p(const vector_insn_info &info1,
                                    const vector_insn_info &info2) {
  return info1.get_ma() != info2.get_ma();
}

static bool possible_zero_avl_p(const vector_insn_info &info1,
                                const vector_insn_info &info2) {
...
second_ratio_invalid_for_first_sew_p(const vector_insn_info &info1,
                                     const vector_insn_info &info2) {
  return calculate_vlmul(info1.get_sew(), info2.get_ratio()) == LMUL_RESERVED;
}
...
static bool second_sew_less_than_first_sew_p(const vector_insn_info &info1,
                                             const vector_insn_info &info2) {
  return info2.get_sew() < info1.get_sew();
}

static bool first_sew_less_than_second_sew_p(const vector_insn_info &info1,
                                             const vector_insn_info &info2) {
  return info1.get_sew() < info2.get_sew();
}
...
/* return 0 if LMUL1 == LMUL2.
   ...
   return 1 if LMUL1 > LMUL2.  */
static int compare_lmul(vlmul_type vlmul1, vlmul_type vlmul2) {
  if (vlmul1 == vlmul2)
    return 0;
...
static bool second_lmul_less_than_first_lmul_p(const vector_insn_info &info1,
                                               const vector_insn_info &info2) {
...
static unsigned greatest_sew(const vector_insn_info &info1,
                             const vector_insn_info &info2) {
  return std::max(info1.get_sew(), info2.get_sew());
}

static unsigned first_sew(const vector_insn_info &info1,
                          const vector_insn_info &) {
  return info1.get_sew();
}

static unsigned second_sew(const vector_insn_info &,
                           const vector_insn_info &info2) {
  return info2.get_sew();
}

static vlmul_type first_vlmul(const vector_insn_info &info1,
                              const vector_insn_info &) {
  return info1.get_vlmul();
}

static vlmul_type second_vlmul(const vector_insn_info &,
                               const vector_insn_info &info2) {
  return info2.get_vlmul();
}

static unsigned first_ratio(const vector_insn_info &info1,
                            const vector_insn_info &) {
  return info1.get_ratio();
}

static unsigned second_ratio(const vector_insn_info &,
                             const vector_insn_info &info2) {
...
#define DEF_SEW_LMUL_FUSE_RULE(                                                \
    DEMAND_SEW1, DEMAND_LMUL1, DEMAND_RATIO1, DEMAND_GE_SEW1, DEMAND_SEW2,     \
    DEMAND_LMUL2, DEMAND_RATIO2, DEMAND_GE_SEW2, NEW_DEMAND_SEW,               \
    NEW_DEMAND_LMUL, NEW_DEMAND_RATIO, NEW_DEMAND_GE_SEW, NEW_SEW, NEW_VLMUL,  \
    NEW_RATIO)                                                                 \
...
static bool always_unavailable(const vector_insn_info &,
                               const vector_insn_info &) {
  return true;
}

static bool avl_unavailable_p(const vector_insn_info &info1,
                              const vector_insn_info &info2) {
  return !info2.compatible_avl_p(info1.get_avl_info());
}

static bool sew_unavailable_p(const vector_insn_info &info1,
                              const vector_insn_info &info2) {
...
static bool lmul_unavailable_p(const vector_insn_info &info1,
                               const vector_insn_info &info2) {
...
static bool ge_sew_unavailable_p(const vector_insn_info &info1,
                                 const vector_insn_info &info2) {
  if (!info2.demand_p(DEMAND_LMUL) && !info2.demand_p(DEMAND_RATIO) &&
...
static bool ge_sew_lmul_unavailable_p(const vector_insn_info &info1,
                                      const vector_insn_info &info2) {
...
static bool same_sew_lmul_demand_p(const bool *dems1, const bool *dems2) {
...
static bool propagate_avl_across_demands_p(const vector_insn_info &info1,
                                           const vector_insn_info &info2) {
...
static bool reg_available_p(const insn_info *insn,
                            const vector_insn_info &info) {
...
static bool support_relaxed_compatible_p(const vector_insn_info &info1,
                                         const vector_insn_info &info2) {
  if (fault_first_load_p(info1.get_insn()->rtl()) &&
    ...
    for (set_info *set : sets) {
      if (read_vl_insn_p(set->insn()->rtl())) {
        const insn_info *insn = get_backward_fault_first_load_insn(set->insn());
...
static int count_regno_occurrences(rtx_insn *rinsn, unsigned int regno) {
  int count = 0;
  extract_insn(rinsn);
...
bool avl_info::single_source_equal_p(const avl_info &other) const {
...
bool avl_info::multiple_source_equal_p(const avl_info &other) const {
...
avl_info &avl_info::operator=(const avl_info &other) {
  m_value = other.get_value();
  m_source = other.get_source();
  return *this;
}

bool avl_info::operator==(const avl_info &other) const {
  if (!m_value)
...
bool avl_info::has_non_zero_avl() const {
  if (has_avl_imm())
...
bool vl_vtype_info::operator==(const vl_vtype_info &other) const {
...
bool vl_vtype_info::same_avl_p(const vl_vtype_info &other) const {
...
bool vl_vtype_info::same_vtype_p(const vl_vtype_info &other) const {
  return get_sew() == other.get_sew() && get_vlmul() == other.get_vlmul() &&
         get_ta() == other.get_ta() && get_ma() == other.get_ma();
}

bool vl_vtype_info::same_vlmax_p(const vl_vtype_info &other) const {
...
bool vector_insn_info::operator>=(const vector_insn_info &other) const {
...
void vector_insn_info::parse_insn(rtx_insn *rinsn) {
...
void vector_insn_info::parse_insn(insn_info *insn) {
  *this = vector_insn_info();
...
bool vector_insn_info::compatible_p(const vector_insn_info &other) const {
...
bool vector_insn_info::skip_avl_compatible_p(
    const vector_insn_info &other) const {
  gcc_assert(valid_or_dirty_p() && other.valid_or_dirty_p() &&
             "Can't compare invalid demanded infos");
...
bool vector_insn_info::compatible_avl_p(const vl_vtype_info &other) const {
...
bool vector_insn_info::compatible_avl_p(const avl_info &other) const {
  gcc_assert(valid_or_dirty_p() && "Can't compare invalid vl_vtype_info");
  gcc_assert(!unknown_p() && "Can't compare AVL in unknown state");
...
bool vector_insn_info::compatible_vtype_p(const vl_vtype_info &other) const {
  gcc_assert(valid_or_dirty_p() && "Can't compare invalid vl_vtype_info");
...
bool vector_insn_info::compatible_p(const vl_vtype_info &curr_info) const {
...
bool vector_insn_info::available_p(const vector_insn_info &other) const {
  return *this >= other;
}
...
void vector_insn_info::fuse_avl(const vector_insn_info &info1,
  ...
  if (info1.demand_p(DEMAND_AVL)) {
    if (info1.demand_p(DEMAND_NONZERO_AVL)) {
      if (info2.demand_p(DEMAND_AVL) && !info2.demand_p(DEMAND_NONZERO_AVL)) {
...
void vector_insn_info::fuse_sew_lmul(const vector_insn_info &info1,
                                     const vector_insn_info &info2) {
...
void vector_insn_info::fuse_tail_policy(const vector_insn_info &info1,
                                        const vector_insn_info &info2) {
...
void vector_insn_info::fuse_mask_policy(const vector_insn_info &info1,
                                        const vector_insn_info &info2) {
...
vector_insn_info vector_insn_info::merge(const vector_insn_info &merge_info,
...
bool vector_insn_info::update_fault_first_load_avl(insn_info *insn) {
...
void vector_insn_info::dump(FILE *file) const {
  fprintf(file, "[");
  if (uninit_p())
...
void vector_infos_manager::create_expr(vector_insn_info &info) {
...
size_t vector_infos_manager::get_expr_id(const vector_insn_info &info) const {
  for (size_t i = 0; i < vector_exprs.length(); i++)
    if (*vector_exprs[i] == info)
      return i;
  gcc_unreachable();
...
auto_vec<size_t> vector_infos_manager::get_all_available_exprs(
...
size_t vector_infos_manager::expr_set_num(sbitmap bitdata) const {
  size_t count = 0;
  for (size_t i = 0; i < vector_exprs.length(); i++)
...
void vector_infos_manager::release(void) {
...
void vector_infos_manager::create_bitmap_vectors(void) {
...
void vector_infos_manager::free_bitmap_vectors(void) {
...
void vector_infos_manager::dump(FILE *file) const {
  basic_block cfg_bb;
  rtx_insn *rinsn;
...
void pass_vsetvl::compute_local_backward_infos(const bb_info *bb) {
...
/* Return true if a dem_info is required to transition from curr_info to
   require before INSN.  */
bool pass_vsetvl::need_vsetvl(const vector_insn_info &require,
...
void pass_vsetvl::transfer_before(vector_insn_info &info,
...
void pass_vsetvl::emit_local_forward_vsetvls(const bb_info *bb) {
  auto &block_info = m_vector_manager->vector_block_infos[bb->index()];
  if (block_info.local_dem.empty_p())
    return;
...
/* We almost enable all cases in get_backward_fusion_type, this function
   ...
   this method.  */
bool pass_vsetvl::hard_empty_block_p(const bb_info *bb,
                                     const vector_insn_info &info) const {
...
bool pass_vsetvl::backward_demand_fusion(void) {
      ...
      FOR_EACH_EDGE (e, ei, cfg_bb->preds)
	{
      ...
      if (block_info.reaching_out.unknown_p())
        ...
        if (prop.has_avl_reg() && !vlmax_avl_p(prop.get_avl())) {
          ...
          if (set)
            block_info.reaching_out.set_avl_info(avl_info(prop.get_avl(), set));
        }
        ...
        if (block_info.reaching_out.compatible_p(prop)) {
          ...
          new_info.set_dirty(block_info.reaching_out.dirty_with_killed_avl_p());
          block_info.probability += curr_block_info.probability;
        } else {
        ...
        if (propagate_avl_across_demands_p(prop, block_info.reaching_out) &&
            !reg_available_p(crtl->ssa->bb(e->src)->end_insn(), new_info))
          continue;
...
bool pass_vsetvl::forward_demand_fusion(void) {
...
void pass_vsetvl::demand_fusion(void) {
  bool changed_p = true;
  while (changed_p) {
    changed_p = false;
  ...
  while (changed_p) {
    ...
    compute_available(
        m_vector_manager->vector_comp, m_vector_manager->vector_kill,
        m_vector_manager->vector_avout, m_vector_manager->vector_avin);
...
bool pass_vsetvl::cleanup_illegal_dirty_blocks(void) {
  bool changed_p = false;
  for (const bb_info *bb : crtl->ssa->bbs()) {
    basic_block cfg_bb = bb->cfg_bb();
    const auto &prop =
...
void pass_vsetvl::prune_expressions(void) {
  for (const bb_info *bb : crtl->ssa->bbs()) {
    if (m_vector_manager->vector_block_infos[bb->index()]
...
void pass_vsetvl::compute_local_properties(void) {
  ...
  for (const bb_info *bb : crtl->ssa->bbs()) {
    ...
    for (size_t i = 0; i < m_vector_manager->vector_exprs.length(); i++) {
      ...
      if (local_dem.real_dirty_p() || local_dem.valid_p() ||
          local_dem.unknown_p() || has_vsetvl_killed_avl_p(bb, local_dem))
...
bool pass_vsetvl::can_refine_vsetvl_p(const basic_block cfg_bb,
                                      const vector_insn_info &info) const {
...
bool pass_vsetvl::commit_vsetvls(void) {
  ...
  for (int ed = 0; ed < NUM_EDGES(m_vector_manager->vector_edge_list); ed++) {
    for (size_t i = 0; i < m_vector_manager->vector_exprs.length(); i++) {
      ...
      if (bitmap_bit_p(m_vector_manager->vector_insert[ed], i)) {
        const vector_insn_info *require = m_vector_manager->vector_exprs[i];
...
void pass_vsetvl::pre_vsetvl(void) {
  /* Compute entity list.  */
  prune_expressions();
...
void pass_vsetvl::propagate_avl(void) const {
  ...
  for (const bb_info *bb : crtl->ssa->bbs()) {
    for (insn_info *insn : bb->real_nondebug_insns()) {
        ...
        if (vsetvl_insn_p(def_insn->rtl())) {
          ...
          if (def_info.get_ratio() == info.get_ratio()) {
            if (vlmax_avl_p(def_info.get_avl())) {
              info.set_avl_info(avl_info(def_info.get_avl(), nullptr));
              rtx new_pat = gen_vsetvl_pat(VSETVL_NORMAL, info, vl);
              validate_change(insn->rtl(), &PATTERN(insn->rtl()), new_pat,
                              false);
              continue;
            }
            if (def_info.has_avl_imm() || rtx_equal_p(avl, vl)) {
              info.set_avl_info(avl_info(avl, nullptr));
              emit_vsetvl_insn(VSETVL_DISCARD_RESULT, EMIT_AFTER, info,
                               NULL_RTX, insn->rtl());
      ...
      if (vsetvl_insn_p(insn->rtl())) {
        ...
        if (vlmax_avl_p(avl))
          continue;
        rtx new_pat = gen_vsetvl_pat(VSETVL_DISCARD_RESULT, info, NULL_RTX);
        if (!set->has_nondebug_insn_uses()) {
          validate_change(insn->rtl(), &PATTERN(insn->rtl()), new_pat, false);
          continue;
        }
      }
    }
...
void pass_vsetvl::compute_probabilities(void) {
...
void pass_vsetvl::lazy_vsetvl(void) {
  if (dump_file)
    fprintf(dump_file,
...
unsigned int pass_vsetvl::execute(function *) {
